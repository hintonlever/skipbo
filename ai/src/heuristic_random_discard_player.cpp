...
Card HeuristicRandomDiscardPlayer::get_card(const GameState& state, const Move& move) {
...
struct HRDChainResult {
    Move first_move;
    int skipbos_used;
    bool found;
};

static HRDChainResult find_chain_to_stock(const GameState& state, int pile_idx) {
    int cp = state.current_player;
    const auto& me = state.players[cp];
...
Move HeuristicRandomDiscardPlayer::choose_move(const GameState& state,
    ...
    {
        HRDChainResult best_chain = {{}, 100, false};
    ...
    // --- PRIORITY 6: Discard (RANDOM) ---
    if (!discard_moves.empty()) {
        // Filter out skipbo discards if possible
        std::vector<Move> non_skipbo_discards;
        for (const auto& m : discard_moves) {
            Card card = get_card(state, m);
            if (!is_skipbo(card)) {
                non_skipbo_discards.push_back(m);
            }
        }

        if (!non_skipbo_discards.empty()) {
            std::uniform_int_distribution<int> dist(0, static_cast<int>(non_skipbo_discards.size()) - 1);
            return non_skipbo_discards[dist(rng_)];
        }

        // Forced to discard a skipbo - pick randomly
        std::uniform_int_distribution<int> dist(0, static_cast<int>(discard_moves.size()) - 1);
        return discard_moves[dist(rng_)];
    }
